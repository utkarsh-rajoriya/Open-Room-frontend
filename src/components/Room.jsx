    ...
    if (stompClientRef.current?.connected) {
      try {
        await fetch(`${baseUrl}/send/${roomId}`, {
          ...
          credentials : "include",
          body: JSON.stringify({ text: message, ai }),
        });
        setMessage("");
      } catch (err) {
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center p-4 text-white">
      ...
      <div className="absolute top-0 left-0 w-full h-[90vh] z-0">
        <Aurora colorStops={["#17153B", "#009fff", "#433D8B"]} blend={0.02} amplitude={0.4} speed={0.8} />
      </div>
      ...
      <div className="relative w-full mt-16 h-[calc(100%-4rem)] max-w-4xl mx-auto flex flex-col z-10 bg-black/20 backdrop-blur-lg border border-white/10 rounded-2xl overflow-hidden">
        ...
        <main ref={chatContainerRef} className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-6">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-end gap-3 ${msg.type === "sent" ? "justify-end" : "justify-start"}`}>
              {msg.type === "received" && (
                <img src={msg.picture || `https://i.pravatar.cc/150?u=${msg.name}`} alt={msg.name} className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
              )}
              <div className={`flex flex-col max-w-[75%] md:max-w-[70%] ${msg.type === "sent" ? "items-end" : "items-start"}`}>
                ...
                <div className={`px-4 py-2 text-white break-words ${msg.type === "sent" ? "bg-blue-600 rounded-t-xl rounded-bl-xl" : "bg-zinc-700 rounded-t-xl rounded-br-xl"}`}>
                  <p>{msg.text}</p>
                </div>
        ...
        <footer className="flex-shrink-0 border-t border-white/10">
          <form className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4" onSubmit={(e) => { e.preventDefault(); handleSend(); }}>
            {/* AI Toggle Button */}
            <button
              type="button"
